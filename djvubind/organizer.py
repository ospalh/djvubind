...
class Page:
    ...
    def get_dpi(self):
        ...
        self.dpi = int(
            utils.execute(
                'identify -ping -units PixelsPerInch -format %x "{0}"'.format(
                    self.path)).decode('ascii'))
        # Looks like imagemagick has changed once more. The Right
        # Thing would be to do a version check. Just make it work with
    ...
    def is_bitonal(self):
        ...
        if utils.execute('identify -ping "{0}"'.format(self.path)).decode('utf8').find('1-bit') == -1:
            self.bitonal = False
        else:
            if int(utils.execute('identify -ping -format %z "{0}"'.format(self.path)).decode('utf8')) != 1:
